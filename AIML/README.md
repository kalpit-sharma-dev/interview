# AIML — Interview & Engineering Guides

## Personalized — Kalpit Sharma

| Doc | Purpose |
|-----|---------|
| [**kalpit-sharma-interview-guide.md**](./kalpit-sharma-interview-guide.md) | **Start here** — role fit, STAR stories, 6-week plan, resume positioning |
| [agentic-genai-engineer-coding-practice.md](./agentic-genai-engineer-coding-practice.md) | Coding exercises + reference solutions |

**Best-fit roles (see personalized guide):** AI Engineering Manager · Staff/Principal GenAI · Director/Head (BFSI AI) — **not** mid-level GenAI IC or pure DS.

---

## Agentic / Generative AI Engineer (IC — enterprise RAG & agents)

| Doc | Purpose |
|-----|---------|
| [agentic-genai-engineer-interview-prep.md](./agentic-genai-engineer-interview-prep.md) | Main prep: RAG, agents, DE/DS proof, Azure, evals |
| [agentic-genai-engineer-cheat-sheet.md](./agentic-genai-engineer-cheat-sheet.md) | One-page review |
| [agentic-genai-engineer-mock-interview-qa.md](./agentic-genai-engineer-mock-interview-qa.md) | Full sample answers |
| [agentic-genai-engineer-coding-practice.md](./agentic-genai-engineer-coding-practice.md) | Hands-on drills + solutions |

## AI agents (technical depth)

| Doc | Purpose |
|-----|---------|
| [guide-to-ai-agent.md](./guide-to-ai-agent.md) | End-to-end agent engineering |
| [master-ai-agent-guide.md](./master-ai-agent-guide.md) | Production architecture, LangGraph, K8s |

## AI Engineering Manager (product company)

| Doc | Purpose |
|-----|---------|
| [ai-engineering-manager-interview-prep.md](./ai-engineering-manager-interview-prep.md) | Leadership, Vertex, MLOps, roadmaps |
| [ai-em-interview-cheat-sheet.md](./ai-em-interview-cheat-sheet.md) | One-page EM review |
| [ai-em-mock-interview-qa.md](./ai-em-mock-interview-qa.md) | EM mock answers |

## Tower Research — AI Operations Manager

| Doc | Purpose |
|-----|---------|
| [TowerResearch/Tower_AI_Ops_Manager_Interview_Prep.md](./TowerResearch/Tower_AI_Ops_Manager_Interview_Prep.md) | AI Ops / FinOps |
| [TowerResearch/Tower_AI_Ops_Mock_Interview.md](./TowerResearch/Tower_AI_Ops_Mock_Interview.md) | Mock scenarios |
| [TowerResearch/Tower_AI_Ops_Deep_Dives.md](./TowerResearch/TowerResearch/Tower_AI_Ops_Deep_Dives.md) | Bedrock cost, vendors |
| [TowerResearch/Tower_AI_Ops_Flashcards.md](./TowerResearch/Tower_AI_Ops_Flashcards.md) | Drill cards |

---

## Role → doc routing

| You are targeting… | Primary docs |
|--------------------|--------------|
| **Kalpit — any senior AI/banking role** | [kalpit-sharma-interview-guide.md](./kalpit-sharma-interview-guide.md) |
| **Agentic / GenAI IC (Staff+)** | agentic-genai-engineer-* + guide + master + coding practice |
| **AI Engineering Manager** | ai-engineering-manager-* + guide + master |
| **AI Ops / FinOps (Tower)** | TowerResearch/* |
